...
def edit_store_item(request, pk):
    ...
    if request.method == "POST":
        ...
        if (
            ...
            try:
                with transaction.atomic():
                    store_item = store_item_form.save()
                    selected_section = store_item_form.cleaned_data.get("section")
                    for sec in Section.objects.filter(items=store_item):
                        sec.items.remove(store_item)
                    if selected_section:
                        selected_section.items.add(store_item)
                    ...
                    for i, form in enumerate(variation_formset.forms):
        ...
        section_initial = {}
        linked_section = store_item.section.first()
        if linked_section:
            section_initial["section"] = linked_section.pk
        store_item_form = StoreItemForm(instance=store_item, initial=section_initial)
